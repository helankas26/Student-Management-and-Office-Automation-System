...
private AnchorPane anchorPaneMainCenter;
    ...
    private void btnSearchOnAction(ActionEvent event) {
        if (!txtTeacherID.getText().isEmpty()) {
            ...
            if (teacherDAO.getDeactivateTeacher(teacher)) {
                setFields();
            } else {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                Stage stage = (Stage) alert.getDialogPane().getScene().getWindow();
                ...
                alert.setTitle("Warning");
                alert.setHeaderText(null);
                alert.setContentText("Invalid TeacherID or active Teacher");
                alert.showAndWait();
            }
        ...
        if (!txtTeacherID.getText().isEmpty()) {
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                ...
                if (teacherDAO.deleteTeacher(teacher)) {
                    ...
                    alert3.setContentText("Invalid TeacherID or active Teacher");
                    alert3.showAndWait();
                }
