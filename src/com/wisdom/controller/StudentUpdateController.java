...
public class StudentUpdateController implements Initializable {
    ...
    @FXML
    private JFXButton btnMakePastStudent;
    @FXML
    ...
    @FXML
    private AnchorPane anchorPaneStudentUpdate;
    ...
    @FXML
    private void btnMakePastStudentOnAction(ActionEvent event) {
    ...
    private void btnSearchOnAction(ActionEvent event) {
        if (!txtStudentID.getText().isEmpty()) {
            ...
            if (studentDAO.getStudent(student)) {
                setFields();
            } else {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                Stage stage = (Stage) alert.getDialogPane().getScene().getWindow();
